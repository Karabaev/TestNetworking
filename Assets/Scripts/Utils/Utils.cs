...
namespace Aboba.Utils
{
  ...
  public static class Utils
  {
    ...
    {
      return gameObject.GetComponent<T>().IsNull() ??
             throw new NullReferenceException($"Component {typeof(T).Name} not found in object {gameObject.name}");
    }
    ...
    public static void AddChild(this Transform transform, MonoBehaviour child) => child.transform.SetParent(transform, false);
    ...
    {
      var direction = MathUtils.Direction(transform.position, target);
    ...
    public static T RequireComponentInChild<T>(this Component component, string childName) where T : Component => 
      GetComponentInChild<T>(component, childName) ?? throw new NullReferenceException($"Component {typeof(T).Name} not found on child {childName} of object {component.name}");
    ...
    {
      if(component.transform.parent.IsNull() == null)
        return null;
    ...
    public static T RequireComponentInChildren<T>(this Component component, string name) where T : class 
    {
      var transform = GetChildRecursive(component.transform, name);
      if(transform == null)
        throw new NullReferenceException($"Child {name} was not found in Object {component.name}");

      return transform.RequireComponent<T>()
          ?? throw new NullReferenceException($"Component {typeof(T).Name} not found in children of object {component.name}");
    }
    ...
    public static Transform RequireChildRecursive(this Component component, string name) =>
      component.transform.GetChildRecursive(name) 
...
#region Vectors
    ...
    public static T? IsNull<T>(this T? reference) where T : class
    {
    ...
    public static void ClearListeners(this Button button) => button.onClick.RemoveAllListeners();
    ...
    public static TComponent NewObject<TComponent>(string name, Transform? parent = null) where TComponent : MonoBehaviour
    {
      var obj = new GameObject(name);
      parent.IsNull()?.AddChild(obj);
